...
function App() {
 ...
 // const NoteList = [<div>Hello, world!</div>, <div>Buy, world!</div>]
  const handleClick = () => {
    let ind; 
    const newValues = [...notes, {select: false, title:'taskN'}];
  ...
  console.log(notes.sort(({select: select1}, {select: select2}) => select1 > select2 ? 1 : -1));
  return (
    <div className="App">
      <h2>ToDo:</h2>
      
      {notes.sort(({select: select1}, {select: select2}) => select1 < select2).map((item, index) => 
          <label key={index}>
            <AddNote 
            select={item.select} 
